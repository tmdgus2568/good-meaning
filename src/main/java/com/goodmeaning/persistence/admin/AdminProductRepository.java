...
public interface AdminProductRepository
	...
	static final String SQL = "select p.product_num, p.product_name, p.product_stock, o.option_num, ifnull(o.option_name, '-'), o.option_stock, "
			+ "p.product_price, o.extraprice, "
			+ "SUM(CASE WHEN DATE_FORMAT(c.purchase_date, \"%Y-%m-%d\") = DATE_FORMAT(now(), \"%Y-%m-%d\") THEN c.purchase_quantity "
	...
	// 재고검색(Querydsl)
	public default Predicate makeStockPredicate(PageVO pageVO) {
		String[] keys = pageVO.getType(); 
		Object[] objects = pageVO.getKeyword();
		...
		if (objects[1] != null && !((String) objects[1]).equals(""))
			builder.and(product.productNum.eq(Long.valueOf((String) objects[1])));
		if (objects[2] != null && !((String) objects[2]).equals("") && !(((String) objects[2]).equals("전체")))
			builder.and(product.postingState.eq((String) objects[2]));
