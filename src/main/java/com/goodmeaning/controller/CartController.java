...
//    	session.setAttribute("user", user );
	...
	// 장바구니 수정
	/*
	 * @RequestMapping(value="cart/update", method = RequestMethod.GET) public
	 * String update(Long[] cartNum, int[] amount, HttpSession session) { //
	 * session의 id UserVO user = UserVO.builder().userPhone("01011114444").build();
	 * session.setAttribute("user", user ); //UserVO user =
	 * (UserVO)session.getAttribute("user"); // 레코드의 갯수 만큼 반복문 실행 for(int i=0;
	 * i<cartNum.length; i++){ CartVO vo = cartService.findById(cartNum[i]);
	 * vo.setCartCount(amount[i]); cartService.insertOrUpdate(vo); } return
	 * "redirect:/cartlist"; }
	 */
